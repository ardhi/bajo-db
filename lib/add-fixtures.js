...
async function addFixture (name, spin) {
  ...
  const { schema, connection } = getInfo(name)
  const cfgMem = getConfig('bajoDbMemory')
  if (connection.memory && cfgMem.persistence.collections.includes(schema.name)) {
    log.warn('\'%s\' is a memory persistence collection. Adding records from fixture ignored!', schema.name)
    return
  }
  if (connection.proxy) {
    log.warn('\'%s\' is bound to a proxied connection, skipped!', schema.name)
    return
  }
