...
async function addFixture (name, spin) {
  ...
  // override
  const overrides = await readConfig(`${this.app.bajo.config.dir.base}/app/bajoDb/fixture/override/${schema.name}.*`, { ns: 'bajo', ignoreError: true })
  if (isArray(overrides) && !isEmpty(overrides)) items = overrides
  // extend
  await eachPlugins(async function ({ dir, ns }) {
    const extend = await readConfig(`${dir}/bajoDb/fixture/extend/${schema.name}.*`, { ns, ignoreError: true })
