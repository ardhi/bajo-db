async function multiRelRows ({ schema, records, options = {} }) {
  ...
  const { getSchema, pickRecord, recordFind } = this.bajoDb.helper
  ...
  if (props.length > 0) {
    for (const prop of props) {
      for (const key in prop.rel) {
        ...
        const results = await recordFind(relschema.name, relFilter, relOptions)
        ...
        for (const i in records) {
          ...
          if (res) records[i]._rel[key] = await pickRecord({ record: res, fields, schema: relschema })
        }
      }
    }
  }
