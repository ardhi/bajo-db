async function checkUnique ({ schema, body, id }) {
  ...
  const singles = map(filter(schema.properties, p => !!p.unique), 'name')
  let old = {}
  if (id) old = (await recordGet(schema.name, id, { skipHook: true, thrownNotFound: false })) ?? {}
  for (const s of singles) {
    if (!isSet(body[s])) continue
    if (id && body[s] === old[s]) continue
    const query = set({}, s, body[s])
    const resp = await recordFind(schema.name, { query, limit: 1 }, { skipHook: true })
    if (resp.length !== 0) {
      const details = [{ field: s, error: 'Unique constraint error' }]
      throw error('Unique constraint error', { details })
    }
  }
  const multis = filter(schema.indexes, i => !!i.unique)
