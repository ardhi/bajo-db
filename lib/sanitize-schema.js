...
async function sanitizeSchema (items) {
  const { freeze, log, fatal, importModule, defaultsDeep, join } = this.bajo.helper
  ...
  for (const k in items) {
    ...
    for (const f in items[k]) {
      ...
      let file = `${driver.provider}:/bajoDb/lib/${conn.type}/prop-sanitizer.js`
      ...
      file = `${driver.provider}:/bajoDb/lib/${conn.type}/schema-sanitizer.js`
