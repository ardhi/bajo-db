...
async function sanitizeFeature (item) {
  ...
  const { get, isPlainObject, isArray, reduce } = await importPkg('lodash-es')
  if (isArray(item.feature)) item.feature = reduce(item.feature, (obj = {}, i) => { obj[i] = true; return obj }, {})
...
async function sanitizeSchema (items) {
  ...
  for (const k in items) {
    ...
    for (const f in items[k]) {
      ...
      if (idx > -1) fatal('Field \'id@%s\' is a reserved field and may not be added manually', item.name)
