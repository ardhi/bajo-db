...
async function sanitizeSchema (items) {
  const { freeze, log, fatal, getConfig, importModule, defaultsDeep } = this.bajo.helper
  const { propType } = this.bajoDb.helper
  const { map, keys, findIndex, find, each, merge, pick, isString, get, isPlainObject } = this.bajo.helper._
    ...
    for (const i2 in schema.properties) {
      ...
      if (prop.rel) {
        for (const key in prop.rel) {
          let val = prop.rel[key]
          if (isString(val)) {
            const [rschema, rfield] = val.split(':')
            val = { schema: rschema, propName: rfield }
          }
          val.type = val.type ?? 'one-to-one'
          const rel = find(schemas, { name: val.schema })
          if (!rel) {
            fatal('No schema found for relationship \'%s@%s:%s\'', `${val.schema}:${val.propName}`, schema.name, prop.name)
          }
          const rprop = find(rel.properties, { name: val.propName })
          if (!rprop) fatal('No property found for relationship \'%s@%s:%s\'', `${val.schema}:${val.propName}`, schema.name, prop.name)
          val.fields = val.fields ?? []
          if (['*', 'all'].includes(val.fields)) {
            const relschema = find(schemas, { name: val.schema })
            val.fields = relschema.properties.map(p => p.name)
          }
          if (val.fields.length > 0 && !val.fields.includes('id')) val.fields.push('id')
          for (const f of val.fields) {
            const p = find(rel.properties, { name: f })
            if (!p) fatal('Unknown property for field \'%s\' in relationship \'%s@%s:%s\'', p, `${val.schema}:${val.propName}`, schema.name, prop.name)
          }
          if (val.propName === prop.name) {
