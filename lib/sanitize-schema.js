...
async function sanitizeIndexes (item) {
  const { importPkg, fatal } = this.bajo.helper
  const { isString, isArray } = await importPkg('lodash-es')
...
async function sanitizeSchema (items) {
  ...
  const { map, keys, findIndex, find, each, merge, pick, isString, get, omit } = await importPkg('lodash-es')
    ...
    for (const f in items[k]) {
      ...
      for (const idx in item.properties) {
        ...
        if (prop.index && prop.index.type === 'fulltext') {
          ...
          prop.index.fields = [prop.name]
          prop.index.indexType = 'FULLTEXT'
          item.indexes.push(omit(prop.index, ['type']))
          delete prop.index
        }
        item.properties[idx] = prop
      }
