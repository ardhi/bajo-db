...
async function sanitizeFeature (item) {
  ...
  for (const f of item.feature) {
    ...
    if (!feature) fatal.call(this, 'Unknown feature \'%s@%s\'', f.name, item.name)
...
async function sanitizeIndexes (item) {
  ...
  for (const idx of item.indexes) {
    if (!(typeof idx.fields === 'string' || Array.isArray(idx.fields))) fatal.call(this, 'Only accept array of field names or single string of field name \'%s@%s\'', 'indexes', item.name)
  }
}
...
async function sanitizeFullText (item) {
  ...
  for (const f of item.fullText.fields ?? []) {
    ...
    if (!prop) fatal.call(this, 'Invalid field name \'%s@%s\'', f, item.name)
...
async function sanitizeSchema (items) {
  ...
  for (const k in items) {
    this.log.trace('Load schema: %s (%d)', k, keys(items[k]).length)
    for (const f in items[k]) {
      const item = items[k][f]
      const conn = find(this.connections, { name: item.connection })
      if (!conn) fatal.call(this, 'Unknown connection \'%s@%s\'', item.connection, item.name)
      ...
      let file = `${ns}:/bajoDb/lib/${type}/prop-sanitizer.js`
      ...
      file = `${ns}:/bajoDb/lib/${type}/schema-sanitizer.js`
